...
function benchmark_graph(graph::OptimShortestPaths.DMYGraph, source::Int; trials::Int = TRIALS)
    # Warm-up to make sure compilation cost is excluded.
    OptimShortestPaths.dmy_sssp!(graph, source)
    OptimShortestPaths.simple_dijkstra(graph, source)
...
function grid_graph(size::Int)
    ...
    for i in 1:size
        for j in 1:size
            v = (i - 1) * size + j
            if j < size
                push!(edges, OptimShortestPaths.Edge(v, v + 1, length(edges) + 1))
                push!(weights, rand() * 2 + 1)
            end
            if i < size
                push!(edges, OptimShortestPaths.Edge(v, v + size, length(edges) + 1))
                push!(weights, rand() * 2 + 1)
            end
        end
    end
