...
@testset "Graph Utils Tests" begin
    ...
    @testset "Edge Access Functions" begin
        edges = [OPUS.Edge(1, 2, 1), OPUS.Edge(1, 3, 2), OPUS.Edge(2, 3, 3)]
        weights = [1.0, 2.0, 1.5]
        graph = OPUS.DMYGraph(3, edges, weights)
    ...
    @testset "Graph Statistics" begin
        ...
        self_loop_edges = [OPUS.Edge(1, 1, 1), OPUS.Edge(1, 2, 2)]
        self_loop_weights = [0.5, 1.0]
        self_loop_graph = OPUS.DMYGraph(2, self_loop_edges, self_loop_weights)
