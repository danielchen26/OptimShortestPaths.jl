...
# Test from source 1
source = 1
dmy_dist = OptimShortestPaths.dmy_sssp!(large_graph, source)
dijkstra_dist = OptimShortestPaths.simple_dijkstra(large_graph, source)
...
dmy_reachable = sum(d < OptimShortestPaths.INF for d in dmy_dist)
dijkstra_reachable = sum(d < OptimShortestPaths.INF for d in dijkstra_dist)
