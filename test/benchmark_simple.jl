...
for n in [50, 100, 200, 500, 1000, 2000]
    ...
    OPUS.dmy_sssp!(graph, 1)
    OPUS.simple_dijkstra(graph, 1)
    ...
    t_dmy_total = 0.0
    for _ in 1:runs
        t_dmy_total += @elapsed OPUS.dmy_sssp!(graph, 1)
    end
    ...
    for _ in 1:runs
        t_dijkstra_total += @elapsed OPUS.simple_dijkstra(graph, 1)
    end
...
dmy_dist = OPUS.dmy_sssp!(test_graph, 1)
dijkstra_dist = OPUS.simple_dijkstra(test_graph, 1)
