...
@testset "DMY Algorithm Tests" begin
    ...
    @testset "Deterministic Frontier Ordering" begin
        edges = [
            OPUS.Edge(1, 2, 1), OPUS.Edge(1, 3, 2),
            OPUS.Edge(2, 4, 3), OPUS.Edge(3, 4, 4)
        ]
    ...
    @testset "Bounded DMY Algorithm" begin
        ...
        equal_bound_graph = OPUS.DMYGraph(3,
            [OPUS.Edge(1, 2, 1), OPUS.Edge(2, 3, 2)],
            [1.5, 2.5])
    ...
    @testset "Algorithm Statistics" begin
        edges = [OPUS.Edge(1, 2, 1), OPUS.Edge(2, 3, 2), OPUS.Edge(1, 3, 3)]
        weights = [1.0, 1.0, 3.0]
    ...
    @testset "Edge Cases" begin
        # Single vertex graph
        single_graph = OPUS.DMYGraph(1, OPUS.Edge[], Float64[])
        ...
        graph_disconnected = OPUS.DMYGraph(4, edges_disconnected, weights_disconnected)
        ...
        edges_self = [OPUS.Edge(1, 1, 1), OPUS.Edge(1, 2, 2)]
        weights_self = [0.5, 1.0]
        graph_self = OPUS.DMYGraph(2, edges_self, weights_self)
    ...
    @testset "Large Graph Performance" begin
        ...
        for i in 1:(n-1)
            push!(edges, OPUS.Edge(i, i+1, i))
            push!(weights, 1.0)
        end
        
        # Add some cross edges
        for i in 1:10:n-10
            push!(edges, OPUS.Edge(i, i+10, length(edges)+1))
            push!(weights, 5.0)
        end
